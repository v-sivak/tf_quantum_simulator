...
class SNAPv2(ParametrizedOperator):
    ...
    def compute(self, theta, dangle=None):
        ...
        projectors = tf.broadcast_to(
            self.projectors, batch_shape + self.projectors.shape
        )  # shape=[B,N,N,N]
        selective_rotations = tensor([R, projectors])  # shape=[B,N,2N,2N]
        selective_rotations = tf.reduce_sum(
            selective_rotations, axis=-3
        )  # shape=[B,2N,2N]
...
class SNAPv3(ParametrizedOperator):
    ...
    def rotation_coeffs(self, angle, phase):
        ...
        for k in range(self.N):
            ...
            Nx = tf.math.reduce_sum(
                (1 - tf.one_hot(k, self.N))
                * angle
                * (tf.math.sin(Delta_k * self.T + phase) - tf.math.sin(phase))
                / (Delta_k * self.T + eps),
                axis=-1,
            )
            Nx += angle[:, k] * tf.math.cos(phase[:, k])
            Nx_list.append(Nx)
            Ny = -tf.math.reduce_sum(
                (1 - tf.one_hot(k, self.N))
                * angle
                * (tf.math.cos(Delta_k * self.T + phase) - tf.math.cos(phase))
                / (Delta_k * self.T + eps),
                axis=-1,
            )
