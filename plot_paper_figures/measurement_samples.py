...
ax.pcolormesh(np.transpose(classified_data[:,:1000]), cmap='RdYlGn', 
...
if SAVE_RAIN_OF_BLOOD:
    savename = os.path.join(plot_config.save_root_dir, 
                        r'measurements_statistics\rain_of_blood')
...
if SAVE_AVG_OUTCOME_FIGURE:
    savename = os.path.join(plot_config.save_root_dir, 
                        r'fig4_characterization\average_outcome')
    fig.savefig(savename, fmt='pdf')
# --------------------------------------------------------------------------
# --------------------------------------------------------------------------
...
if SAVE_RAIN_OF_BLOOD_SMALL:
    savename = os.path.join(plot_config.save_root_dir, 
                        r'fig4_characterization\msmt_sample')
    fig.savefig(savename, fmt='pdf')
