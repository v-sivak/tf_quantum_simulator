...
## Changing T1
fig, axes = plt.subplots(1, 2, figsize=(4,2), dpi=600, sharey='row')
ax = axes[0]
...
### Logical rates vs physical rates V1
...
## Changing T1
fig, axes = plt.subplots(1, 2, figsize=(3.38,2), dpi=600, sharey='row')
ax = axes[0]
ax.set_ylim(0,19)
ax.grid(True)
...
### Logical rates vs physical rates V2
...
## Changing T1
fig, ax = plt.subplots(1, 1, figsize=(3.38,2), dpi=600)
ax.set_ylim(0,19)
# ax.set_yscale('log')
# ax.set_xscale('log')
ax.grid(True)
...
ax.text(70, 3.5, r'Pure dephasing, $\gamma_{\phi}^{\,t}$')
ax.text(45, 13, r'Relaxation + excitation, $\gamma_1^{\,t}$')
...
for m in ['gkp_X', 'gkp_Y', 'gkp_Z']: 
    ...
    ax.plot(1/(taus['T1']*1e-6), 1/(taus[m]*1e-6), label=labels[m], marker='>', 
            color=color[m], linestyle='none', markersize=4)
...
for m in ['gkp_X', 'gkp_Y', 'gkp_Z']:
    ...
    ax.plot(1/(taus['T_phi']*1e-6), 1/(taus[m]*1e-6), marker='<', label=labels[m],
            color=color[m], linestyle='none', markersize=4)
